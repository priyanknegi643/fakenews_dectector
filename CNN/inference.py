"""
inference.py — Use the trained CNN+LSTM model to predict on new articles
Usage:
    python inference.py "Paste your article text here …"
    python inference.py --file article.txt
"""
...
def predict(texts: list[str], model, tokenizer) -> list[dict]:
    ...
    for text, prob in zip(texts, probs):
        ...
        results.append({"label": label, "confidence": float(confidence),
                         "real_prob": float(prob), "preview": text[:120]})
    return results
...
def main():
    ...
    group.add_argument("text",  nargs="?",  help="Article text as a string")
    group.add_argument("--file", "-f",      help="Path to a .txt file")
